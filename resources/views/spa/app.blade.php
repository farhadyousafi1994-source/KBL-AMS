...
new Vue({
    ...
    methods: {
        notifyError(error) {
            ...
            const firstError = errors ? Object.values(errors)[0][0] : (error.response && error.response.data && error.response.data.message) || 'Could not save data. Please check database connection and required fields.';
            this.$q.notify({ type: 'negative', message: firstError });
        },
        ...
        async fetchDashboard() {
            try {
                const { data } = await axios.get('/api/spa/dashboard');
        ...
        async saveEmployee() {
            try {
                await axios.post('/api/spa/employees', this.employeeForm);
                this.employeeForm = {};
                this.notifySuccess('Employee saved');
        ...
        async saveAsset() {
            try {
                await axios.post('/api/spa/assets', this.assetForm);
                this.assetForm = {};
        ...
        async assignAsset() {
            try {
                await axios.post(`/api/spa/assets/${this.assignmentForm.asset_id}/assign`, this.assignmentForm);
                this.assignmentForm = {};
                this.notifySuccess('Asset assigned');
                await this.fetchDashboard();
            } catch (error) {
                this.notifyError(error);
            }
